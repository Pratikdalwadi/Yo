...
Unit test status Integration test status  PyPI version
...
🗂️ All file types: parse PDFs of any length, single images, or URLs → see Supported Files
...
👁️ Ground‑truth visuals: optional bounding‑box snippets and full‑page visualizations → see Save Groundings as Images
...
Rate Limits: The library automatically retries requests if you hit the API rate limit. Adjust BATCH_SIZE or MAX_WORKERS if you encounter frequent rate limit errors.
Parsing Failures: If a document fails to parse, an error chunk will be included in the result, detailing the error message and page index.
